...
def fomfp(ats, els, rcScaling=1.0, ns=2, npp=1):
    nat = ats.shape[0]
    rcov = get_rcov(els) * rcScaling
    rc = 10000000.0
    f = fp.fingerprint(nat, ns, npp, rc ,np.zeros((3,3)), ats.transpose(), rcov, nat)
    df = np.transpose(df, axes=(0, 2, 1))
    return f

def smooth_fomfp(ats, els, rcScaling=1.0, ns=2, npp=1):

    f = fomfp(ats, els, rcScaling, ns, npp, returnDerivatives=True)
    f_smooth = np.sum(np.sin(np.outer(f, x_grid)), axis=0)
    return f_smooth

def fomfpall(ats, els, lattice=None, rc=6., rcScaling=1.0, ns=2, npp=1, maxNatSphere=None):
    nat = ats.shape[0]
    rcov = get_rcov(els) * rcScaling
    ...
    #rc = 10000000.0
    if lattice is None:
        f = fpall.fingerprint(maxNatSphere, ns, npp, rc / 2., np.zeros((3,3)), ats.transpose(), rcov, nat)
    else:
        f = fpall.fingerprint(maxNatSphere, ns, npp, rc / 2., lattice.transpose(), ats.transpose(), rcov, nat)
    #df = df.transpose()
...
def smooth_fomfpall(ats, els, lattice=None, rc=6., rcScaling=1.0, ns=2, npp=1, maxNatSphere=None, returnDerivatives=False):
    f, df = fomfpall(ats, els, lattice, rc, rcScaling, ns, npp, maxNatSphere)
