...
!------------------------------------------------------------------------------
  ...
  SUBROUTINE fingerprint_and_derivatives(nat, nat_sphere_max, ns, np, width_cutoff, alat,rxyz,rcov, fp, dfp)
    ...
    REAL(8), DIMENSION(nat_sphere_max*(ns+3*np)), INTENT(OUT) :: fp  !fingerprint vectors
    REAL(8), DIMENSION(nat_sphere_max*(ns+3*np),3,nat),INTENT(OUT) :: dfp !fingerrprint derivatives
    ...
    DO l = 1, norb
      DO iat=1,nat_sphere
        iiat=indat(iat)
        dfp(l,1,iiat)=dfp(l,1,iiat) + devaldr(1,iat,l)
        dfp(l,2,iiat)=dfp(l,2,iiat) + devaldr(2,iat,l)
        dfp(l,3,iiat)=dfp(l,3,iiat) + devaldr(3,iat,l)
      ENDDO
    ENDDO
